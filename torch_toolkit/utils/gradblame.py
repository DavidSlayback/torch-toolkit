...
def grads(l):
    """Call on loss/output to recursively grab gradients along the way"""
    hooks = {}
    ...
    for _, h in hooks:
        h.remove()
    return ArrayDict(tensors=tensorgrads, ins=ingrads, outs=outgrads)
...
def graph(output, params={}):
    """Call on output of grads to get graph of gradient flow w.r.t. parameters"""
...
"""Call on outputs of a model to get a graph of gradients w.r.t. components of model"""
