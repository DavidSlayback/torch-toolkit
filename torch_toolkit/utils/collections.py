...
class AttrDict(dict):
    """Really basic attribute-style-access dict"""
...
@mapping
def torchify_copy(a, device: Optional[Union[th.device, str]] = 'cpu', override_bool: bool = False):
    ...
    """
    if hasattr(a, 'torchify_copy'):
        return a.torchify_copy()
    a = np.asarray(a)
...
@mapping
def numpyify_copy(tensors):
    """Converts an array or a dict of tensors to numpy arrays."""
    if isinstance(tensors, tuple):
        return tuple(numpyify_copy(t) for t in tensors)
...
def arrdict_stack(x, *args, **kwargs):
    ...
    if isinstance(x[0], dict):
        ks = x[0].keys()
        return x[0].__class__({k: arrdict_stack([y[k] for y in x], *args, **kwargs) for k in ks})
...
def arrdict_cat(x, *args, **kwargs):
    ...
    if isinstance(x[0], dict):
        ks = x[0].keys()
        return x[0].__class__({k: arrdict_cat([y[k] for y in x], *args, **kwargs) for k in ks})
    if isinstance(x[0], th.Tensor):
...
def arrdict_clone(t):
    ...
    if hasattr(t, 'clone'):
        return t.arrdict_clone()
    if hasattr(t, 'copy'):
        return t.copy()
    return t


def arrdict_from_dicts(t):
    """Create ArrayDict from dict"""
    if isinstance(t, dict):
        return ArrayDict({k: arrdict_from_dicts(v) for k, v in t.items()})
    return t


def arrdict_to_dicts(t):
    """Create dict of [dicts] from ArrayDict"""
    if isinstance(t, dict):
        return {k: arrdict_to_dicts(v) for k, v in t.items()}
    return t