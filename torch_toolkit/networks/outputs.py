...
class DiscreteHead(ActorHead):
    ...
    def log_prob(self, logits: Tensor, action: Tensor) -> Tensor:
        ...
        """
        if self.num_policies > 1 and (logits.ndim - action.ndim == 2): action = action.unsqueeze(-1)  # Add sub-policy dimension
        return logits.gather(-1, action.unsqueeze(-1)).squeeze()
